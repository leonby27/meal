...
class SignInWithAppleAuthorizationController: NSObject, ASAuthorizationControllerDelegate, ASAuthorizationControllerPresentationContextProviding {
    ...
    // Patched: provide a presentation anchor. Without this, iOS 15+ (and
    // especially iOS 17+ / scene-based Flutter apps with SceneDelegate) never
    // delivers the authorization callback back to the plugin, so the Dart
    // future hangs forever even though the native sheet is shown.
